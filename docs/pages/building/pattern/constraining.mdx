...
For convenience when working with arithmetic expressions, there is a macro `arith_expr` found in [Binius macros](https://github.com/IrreducibleOSS/binius/blob/main/crates/macros/src/lib.rs) that can reduce verbosity. For example, our `expression` constraining `bits` to take values zero or one could be written as
...
The third type of constraint we may apply to columns is flushing. See [here](/basics/arithmetization/m3/definition/#flushing-rules) for context on the role of flushing within arithmetization.

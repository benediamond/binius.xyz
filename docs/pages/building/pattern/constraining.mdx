# Constraining Columns [Ensuring the validity of the witness]
...
Having discussed [virtual oracles](/building/pattern/declaring/#virtual-columns) previously, we focus this page on the other three types of constraints. To clarify, those three constraint types are:
...
Let us show how to construct an arithmetic expression. [Arithmetic expressions in Binius](https://gitlab.com/IrreducibleOSS/binius/-/blob/main/crates/math/src/arith_expr.rs) are symbolic expression trees, providing a convenient and flexible way to represent arithmetic expressions. With expressions structured as a tree, each node represents an operation or an operand.
...
For convenience when working with arithmetic expressions, there is a macro `arith_expr` found in [Binius macros](https://gitlab.com/IrreducibleOSS/binius/-/blob/main/crates/macros/src/lib.rs) that can reduce verbosity. For example, our `expression` constraining `bits` to take values zero or one could be written as
...
The third type of constraint we may apply to columns is flushing. See [here](/basics/arithmetization/matching/definition/#flushing-rules) for context on the role of flushing within arithmetization.

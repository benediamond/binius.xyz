...
// Witness wires
let witness = builder.add_witness();
...
fn prove_preimage_knowledge() {
    ...
    // Preimage (only prover knows this)
    let preimage_value = Word(0xDEADBEEFCAFEBABE);
    w[preimage_wire] = preimage_value;

    // Compute the hash (same logic as in circuit)
    let rotated = preimage_value.rotl(13);
    let mixed = rotated ^ Word(0x1234567890ABCDEF);
    let hash = mixed ^ preimage_value.shr(7);
